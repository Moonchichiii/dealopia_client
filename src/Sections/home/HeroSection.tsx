...
import { usePopularDeals } from '@/hooks/useDeals';
...
const HeroSection: React.FC = () => {
  ...
  const { data: popularDeals, isLoading: dealsLoading } = usePopularDeals(3);
  ...
  const handleSearch = (query: string, filters?: Record<string, any>) => {
    ...
    if (filters?.categories?.length) {
      searchParams.set('category', filters.categories[0].toString());
    }
  ...
  useGSAP(() => {
    ...
    .from(contentRef.current.querySelector('.search-container'), {
      y: 20,
      opacity: 0,
      duration: 0.6,
      ease: 'power3.out'
      ...
      <div className="container mx-auto px-4">
        <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          <div ref={contentRef} className="max-w-2xl">
            ...
            {/* Search Component */}
            <div className="search-container mb-8"></div>
              <SearchComponent
          ...
          <div ref={showcaseRef} className="bg-black/30 rounded-2xl overflow-hidden backdrop-blur-sm shadow-xl">
            <div className="flex items-center gap-3 px-6 py-5 border-b border-white/10">
              ...
              <div className="text-gray-200 font-medium">Popular Deals Near You</div>
            </div>
            
            <div className="p-4">
              {dealsLoading ? (
                [...Array(3)].map((_, i) => (
                  <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-white/5 animate-pulse mb-2 h-20" />
                ))
              ) : (
                popularDeals?.map((deal) => (
                  <Link key={deal.id} to={`/deals/${deal.id}`} className="block">
                    <div className="flex items-center gap-4 p-4 rounded-xl hover:bg-white/5 transition-colors mb-2">
                      <div className="w-12 h-12 bg-[var(--accent-pink)] text-white rounded-lg flex items-center justify-center text-2xl">
                        {deal.category_names?.[0]?.charAt(0) || '🏷️'}
                      </div>
                      <div className="flex-1">
                        <h4 className="font-medium text-white truncate">{deal.title}</h4>
                        <p className="text-sm text-gray-300 truncate">{deal.shop_name}</p>
                      </div>
